...
class HelloFrame(wx.Frame):
    ...
    def __init__(self,*args,**kw):
        ...
        self.pnl = pnl
        st = wx.StaticText(pnl, label="景点识别", pos=(455, 0))
        font = st.GetFont()
        font.PointSize += 10
        font = font.Bold()
        st.SetFont(font)
        ...
        self.SetStatusText("Welcome to Tourist attraction Identification")
...
if __name__ == '__main__':
    ...
    app = wx.App()
    frm = HelloFrame(None, title='Tourist attraction', size=(1000,600))
    frm.Show()
    app.MainLoop()
    # 在显示出分类结果后会报出错误，虽然不影响使用，但还是挺烦人的，不知道问题在哪